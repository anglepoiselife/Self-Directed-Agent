  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f4f8', fontFamily: 'system-ui, sans-serif' }}>
      <form onSubmit={handleSubmit} style={{ background: '#fff', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', width: '360px' }}>
        <h2 style={{ margin: '0 0 1.5rem', textAlign: 'center', color: '#1a1a2e' }}>Sign In</h2>
        {error && <p style={{ color: '#e53e3e', fontSize: '0.85rem', margin: '0 0 1rem' }}>{error}</p>}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <input type="text" placeholder="Email or Username" ref={identifierRef} style={inputStyle} />
          <input type="password" placeholder="Password" ref={passwordRef} style={inputStyle} />
          <button type="submit" onClick={(e) => handleSubmit(e)} disabled={loading} style={{ padding: '10px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '1rem', cursor: 'pointer', opacity: loading ? 0.6 : 1 }}>
            {loading ? 'Signing in...' : 'Sign In'}
          </button>
        </div>
        <p style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.85rem', color: '#666' }}>
          No account? <Link to="/register" style={{ color: '#4f46e5', textDecoration: 'none' }}>Register</Link>
        </p>
      </form>
    </div>
  );
